...
function App() {
  const baseUrl="http://localhost/guia8/";
  ...
  const peticionPut=async()=>{
    ...
    .then(response=>{
      var dataNueva= data;
      dataNueva.map(prodcuto=>{
        if(prodcuto.id===productoSeleccionado.id){
          prodcuto.nombre=productoSeleccionado.nombre;
          prodcuto.existencias=productoSeleccionado.existencias;
        }
      });
      setData(dataNueva);
